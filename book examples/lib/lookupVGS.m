...
%
% VGS = lookupVGS(nch, 'GM_ID', 10, 'VDS', 0.6, 'VSB', 0.1, 'L', 0.1)
% VGS = lookupVGS(nch, 'GM_ID', 10:15, 'VDS', 0.6, 'VSB', 0.1, 'L', 0.1)
% VGS = lookupVGS(nch, 'ID_W', 1e-4, 'VDS', 0.6, 'VSB', 0.1, 'L', 0.1)
% VGS = lookupVGS(nch, 'ID_W', 1e-4, 'VDS', 0.6, 'VSB', 0.1, 'L', [0.1:0.1:0.5])
%
...
% example:
%
% VGS = lookupVGS(nch, 'GM_ID', 10, 'VDB', 0.6, 'VGB', 1, 'L', 0.1)
% VGS = lookupVGS(nch, 'ID_W', 1e-4, 'VDB', 0.6, 'VGB', 1, 'L', 0.1)
%
...
if ~isnan(par.ID_W(1))
    ...
    disp('lookupVGS: Invalid syntax or usage mode! Please type "help lookupVGS".')
    output = [];
    return;      
    end
end
...
if  mode == 1
    VGS = data.VGS;
    ratio = lookup(data, ratio_string, 'VGS', VGS, 'VDS', par.VDS, 'VSB', par.VSB, 'L', par.L);
    ...
    VGS    = par.VGB - VSB;
	VDS    = par.VDB - VSB;
	ratio  = lookup(data, ratio_string, 'VGS', VGS, 'VDS', VDS, 'VSB', VSB, 'L', ones(length(VSB),1)*par.L);
    idx    = isfinite(ratio);
    ratio  = ratio(idx);
    VGS    = VGS(idx);
end    
...
for j = 1:s(2)
    ...
    if strcmp(ratio_string,'GM_ID')
        ...
        if max(ratio_data) > m
            disp('lookupVGS: GM_ID input larger than maximum!')
        end
    end
