    ...
    for i = 1:dim(2)
        for j = 1:length(xdesired)
            ...
            if max( xdesired(j)) > m && strcmp(par.WARNING, 'on')
                s = sprintf('lookup warning: %s_%s input larger than maximum! (output is NaN)', numerator, denominator);
                disp(s);
            end
            ...
            elseif strcmp(numerator,'GM') && (strcmp(denominator,'CGG') || strcmp(denominator,'CGG'))
                ...
                if crossings > 1
                    ...
                    disp('*** Example: lookup(nch, ''ID_W'', ''GM_GDS'', gm_gds, ''VGS'', nch.VGS(10:end))');
                    disp(' ')
                    figure(1000)
