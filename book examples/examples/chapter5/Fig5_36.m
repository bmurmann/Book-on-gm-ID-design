% 5.6.2   gm/ID across corners for 2 biasing scenarios
clearvars;
close all;
addpath ../../lib
...
delta_slow = 100*(gm_ID_slow_hot - gm_ID)./gm_ID;
delta_fast = 100*(gm_ID_fast_cold - gm_ID)./gm_ID;

h = figure(1);
subaxis(2,2,1, 'Spacing', 0.13, 'MarginBottom', 0.13, 'MarginTop', 0.06, 'MarginLeft', 0.1, 'MarginRight', 0.02)
semilogx(ID_W, gm_ID, 'k', 'linewidth', 2)
t=title('Constant {\itI_D}')
set(t, 'fontsize', 9, 'fontweight', 'normal')
hold on;
semilogx(ID_W, gm_ID_slow_hot, 'k:', 'linewidth', 1)
semilogx(ID_W, gm_ID_fast_cold, 'k--', 'linewidth', 1)
xlabel({'{\itI_D}/{\itW} (A/\mum)', '(a)'});
ylabel('{\itg_m}/{\itI_D}  (S/A)');
axis([0.5e-6 max(ID_W) 3 35])
set(gca, 'xtick', [1e-6 1e-5 1e-4]);
legend('Nom.', 'Slow, hot', 'Fast, cold')
grid;
...
subaxis(2,2,2)
semilogx(gm_W, gm_ID, 'k', 'linewidth', 2)
t=title('Constant {\itg_m}');
set(t, 'fontsize', 9, 'fontweight', 'normal')
hold on;
semilogx(gm_W, gm_ID_slow_hot, 'k:', 'linewidth', 1)
semilogx(gm_W, gm_ID_fast_cold, 'k--', 'linewidth', 1)
xlabel({'{\itg_m}/{\itW}  (S/\mum)', '(b)'});
ylabel('{\itg_m}/{\itI_D}  (S/A)');
axis([1e-5 1.2*max(gm_W) 3 35])
...
%format_and_save(h, 'Fig5_36', 'W', 5.3, 'H', 5)
