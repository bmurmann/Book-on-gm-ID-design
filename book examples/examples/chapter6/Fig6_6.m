% Fig. 6.6 OTA feedback factor optimization (fixed fanout)
clearvars;
close all;
addpath ../../lib
load 65nch.mat
...
plot(gm_ID, beta(1,:)/beta_max(1), 'k-', 'linewidth', 2);
hold on;
plot(gm_ID, beta(2,:)/beta_max(2), 'k-');
plot(gm_ID, beta(3,:)/beta_max(3), 'k--');
plot(gm_ID, beta(4,:)/beta_max(4), 'k-.');
...
legend('{\itG} = 1', '{\itG} = 2', '{\itG} = 4', '{\itG} = 8', 'location', 'southwest');
grid;
...
axis([min(gm_ID) max(gm_ID)+1 0 30])
grid;

%format_and_save(h, 'Fig6_6', 'H', 5)
